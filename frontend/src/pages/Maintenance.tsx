...
export default function Maintenance() {
  const [rows, setRows] = useState<MaintenancePreview[]>([]);
  ...
  const load = useCallback(async () => {
    ...
    setErr(null);
    try {
      const data = await previewMaintenance();
      setRows(data);
    } catch (e: any) {
  ...
  const materialize = async () => {
    ...
    setErr(null);
    try {
      const res = await materializeMaintenance();
      setMsg(`Created ${res.created} task(s).`);
      await load();
    } catch (e: any) {
      ...
      <section className="rounded-2xl shadow overflow-x-auto">
        <div className="p-4 border-b">
          ...
          <p className="text-xs opacity-60">
            These would be created if you click “Generate Next Occurrences”.
          </p>
        </div>
            ...
            {rows.length === 0 ? (
              ...
              rows.map((r, i) => (
                <tr key={i} className="border-t">
                  ...
                  <td className="px-4 py-3">
                    {r.template.due_at
                      ? new Date(r.template.due_at).toLocaleString()
                      : "—"}
                  </td>
                  <td className="px-4 py-3">
                    {new Date(r.will_create.due_at).toLocaleString()}
                  </td>
                </tr>
              ))
            )}
